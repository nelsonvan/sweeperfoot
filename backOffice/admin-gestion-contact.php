...
if (isset($_GET['action']) && $_GET['action'] == 'suppression' && isset($_GET['id'])){
    ...
    $contact_delete = $bdd->prepare("DELETE FROM contact WHERE id_contact = :id_contact");
    // Je recupere l'id qui se trouve dans l'URL
    $contact_delete->bindValue(':id_contact', $_GET['id'], PDO::PARAM_INT);
...
while ($contact = $resultat->fetch(PDO::FETCH_ASSOC)){
  ...
  $contenu .= '<tr>';
  $contenu .= '<td scope="col" class="array-article text-center">' . $contact['id_contact'] . '</td>';
$contenu .= '<td scope="col" class="array-article  text-center">' . $contact['contactFirstName'] . '</td>';
$contenu .= '<td scope="col" class="array-article  text-center text-justify">' . $contact['contactEmail'] . '</td>';
$contenu .= '<td scope="col" class="array-article  text-center text-justify">' . $contact['contactMessage'] . '</td>';
...
$contenu .= '<td  scope="col" class="array-article  text-center"><a class="return"  href="?action=suppression&id=' . $contact['id_contact'] . '" onClick="return confirm(\'Etes-vous sûr ?\');"><i
...
<div class="container mt-5">
    ...
    <table class="table table-dark table-bordered mt-5">
            <thead>
                <tr>
                    <th scope="col" class="array-user text-center">N° Contact</th>
                    <th scope="col" class="array-user  text-center">prénom</th>
                    <th scope="col" class="array-user text-center">Email</th>
                    <th scope="col" class="array-user text-center">Message</th>
